...
@test "Test pthread_cancel with libunwind compatibility" {
    # This test verifies pthread_cancel works without segfaults with all combinations
    # including libunwind (previously caused segfaults, now should be fixed)
...
EOF
    ...
    # Test with libc++, compiler-rt and libunwind - should work (bug fixed)
    run clang++-$VERSION -stdlib=libc++ -unwindlib=libunwind --rtlib=compiler-rt -o "${BATS_TMPDIR}/pthread_test5" "${BATS_TMPDIR}/pthread_cancel_test.cpp"
    assert_success "Compilation with libc++, compiler-rt and libunwind failed"
    
    run timeout 5s "${BATS_TMPDIR}/pthread_test5"
    assert_success "Execution with libc++, compiler-rt and libunwind should not segfault"

    # Test with libstdc++, compiler-rt and libunwind - should work (bug fixed)
    run clang++-$VERSION -unwindlib=libunwind --rtlib=compiler-rt -o "${BATS_TMPDIR}/pthread_test6" "${BATS_TMPDIR}/pthread_cancel_test.cpp"
    assert_success "Compilation with libstdc++, compiler-rt and libunwind failed"
    
    run timeout 5s "${BATS_TMPDIR}/pthread_test6"
    assert_success "Execution with libstdc++, compiler-rt and libunwind should not segfault"
}
